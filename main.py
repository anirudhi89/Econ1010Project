...
usa_data.to_csv('results/USA_wage.csv', index=False)
immigrants_data.to_csv('results/immigrant_wage.csv', index=False)
...
pd.Series(avg_USA_wage).to_csv('results/avg_USA_wage.csv', header=['Average_Wage'])
for country, avg_wage in avg_immigrant_wages.items():
    pd.Series(avg_wage).to_csv(f'results/avg_{country}_wage.csv', header=['Average_Wage'])
